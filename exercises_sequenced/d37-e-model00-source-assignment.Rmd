...
Induced
The properties of the components come from the same batch and if multiple measurements on the same part return different values, then the variability is most likely induced by the measurement process.
...
Real
The properties of multiple components from different batches will not be identical.
...
- mixture of both
- real variability arises from different manufacturing batches, however since this is a destructive process, you are unable to reduce induced variability
...
- Which is larger: The variability due to noise, or the variability due to deviation?
  - The variability due to deviation is larger.
...
- No because there is a lot of variability 
...
- How does `Y_lo` compare with `Y_mean`?
  - Y_lo is significantly lower than Y_mean
...
- What features about the data collection / dataset `df_meas` enable the calculation of `Y_lo_improved`?
  - We needed multiple observations id_meas on each sample id_sample
