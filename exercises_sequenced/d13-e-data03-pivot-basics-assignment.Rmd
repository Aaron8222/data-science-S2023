...
df_q3_res <-
  df_q3 %>%
  pivot_wider(
    names_from = id, values_from = value, names_prefix = "X-"
  )
...
df_q4_res <-
  df_q4 %>%
  pivot_longer(
    col = c("A", "B", "C"), names_to = "key", values_to = "X-0", values_drop_na = TRUE
  )
...
df_q6_res <-
  df_q6 %>%
  pivot_longer(
    col = c("9", "8", "7"), names_to = "X-1", values_to = "key", values_drop_na = TRUE, 
    names_transform = list("X-1" = as.numeric, "X-0" = as.numeric)
  )
