...
- (Take notes here!) There are a total of 6 columns and 122 rows in the
  dataset. The values range greatly from 10^-6 to 10^7.
...
base_mse <- mse(fit_baseline, df_train)
base_rsq <- rsquare(fit_baseline, df_train)
cheat_mse <- mse(fit_cheat, df_train)
cheat_rsq <- rsquare(fit_cheat, df_train)
non_mse <- mse(fit_nonphysical, df_train)
non_rsq <- rsquare(fit_nonphysical, df_train)

df_errors = data.frame(model = c("fit_baseline", "fit_cheat", "fit_nonphysical"),
                       mse = c(base_mse, cheat_mse, non_mse),
                       rsquare = c(base_rsq, cheat_rsq, non_rsq))
df_errors
```

    ##             model        mse     rsquare
    ## 1    fit_baseline 0.15153384 0.348376352
    ## 2       fit_cheat 0.05194415 0.776630527
    ## 3 fit_nonphysical 0.23224544 0.001301518

**Observations**:

- Which model is *most accurate*? Which is *least accurate*?
  - Fit_cheat is the most accurate and fit_nonphysical is the least
    accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - It is a continuous numeric variable. It is an effective predictor
    because the avg_T and T_norm are both a measure of temperature.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - We would not have access to avg_T so it is not a valid predictor.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - It is a nominal categorical variable. It does not have any physical
    meaning.
...
fit_q4 <- df_train %>% 
  lm(formula = T_norm ~ x + L + W + U_0 + T_f + rho_f + mu_f + C_fp + d_p + I_0)
...
- *Note*: You don’t just have to fiddle with `formula`! Remember that
  ...
  channel or properties/state of the fluid. The states include the bulk
  velocity, inlet temperature, and radiation intensity. The fluid
  properties include density, viscosity, heat capacity, and particle
  size.
...
```
    ...
    ##        fit     lwr      upr
    ## 1 1.876638 1.45685 2.296426

**Recommendation**:

- How much do you trust your model? Why?
  - I do not trust this model a lot because the r-square is very low for
    df_train and df_validate.
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - You want to use a prediction interval because you want to capture
    new observations and not mean trends. You want to predict the range
...
- Are there any other recommendations you would provide?
  - Considering the r-square is so low, other variables should also be
    tightly controlled by the design team.
