...
- (Take notes here!)
There are a total of 6 columns and 122 rows in the dataset. The values range greatly from 10^-6 to 10^7.
...
- Which model is *most accurate*? Which is *least accurate*?
  - Fit_cheat is the most accurate and fit_nonphysical is the least accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective predictor?
  - It is a continuous numeric variable. It is an effective predictor because the avg_T and T_norm are both a measure of temperature.
...
- What *Category* of variable is `idx`? Does it have any physical meaning?
  - It is a nominal categorical variable. It does not have any physical meaning.
...
It seems the channel dimension parameters (x, L, W) are useful predictors of T_norm. The initial state of the system such as bulk velocity, inlet temp, and radiation intensity (U_0, T_f, I_0) seem to be useful predictors as well. Finally, the properties of the fluid, such as the density, viscosity, heat capacity, particle diameter (rho_f, mu_f, C_fp, d_p) also seem to be important in predicting T_norm

- *Note*: You don't just have to fiddle with `formula`! Remember that you have a whole toolkit of *EDA* tools
The useful predictors that seem to be useful are either related to the dimensions of of the channel or properties/state of the fluid. The states include the bulk velocity, inlet temperature, and radiation intensity. The fluid properties include density, viscosity, heat capacity, and particle size.
...
- Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
  - q4 tends to be more accurate because predicted T_norm is closer to the actual T_norm. 
...
- How much do you trust your model? Why?
  - I do not trust this model a lot because the r-square is very low for df_train and df_validate.
- What kind of interval---confidence or prediction---would you use for this task, and why?
  - You want to use a prediction interval because you want to capture new observations and not mean trends. You want to predict the range of possible T_norm values.
- What fraction of validation cases lie within the interval you predict? How does this compare with `pr_level`?
  - The fraction of validation cases that lie within the interval the model predicted was 0.983. This is higher than the specified pr_level of 0.8.
...
- Are there any other recommendations you would provide?
  - Considering the r-square is so low, other variables should also be tightly controlled by the design team.
