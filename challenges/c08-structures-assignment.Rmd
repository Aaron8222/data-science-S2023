...
- Can you confidently conclude that `POF < 0.03`? Why or why not.
  - I can't confidently conclude that POF < 0.03 because again the sample size is so small.
...
- Assuming your scopus is the probability of failure `POF` defined above, does your estimate exhibit real variability, induced variability, or both?
  - Our scopus has induced variability and come from the fact that the sample size is small. There is no real variability the distribution of strength is not changing.
...
- Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
  - The confidence interval does not account for the uncertainty arising from limited physical tests because the standard error you compute is based on the sample size generated from the monte carlo method (In this case, it's 1000) and therefore does not account for the number of physical tests.
...
- Can you confidently conclude that `POF < 0.03`? Why or why not?
  - We cannot confidently conclude that POF < 0.03 because because based on the confidence interval, the POF could be in the range of 0.00138 to 0.0458, which is below and above this threshold.
