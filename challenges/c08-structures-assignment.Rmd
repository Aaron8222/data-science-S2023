...
- Does the confidence interval above account for uncertainty arising from *Monte Carlo approximation* of the POF? Why or why not?
  - No because the bootstrap method is different from the monte carlo method. Instead of generating random samples from a probabilty distribution, bootstrap works by resampling the available data.
- Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
...
- Can you confidently conclude that `POF < 0.03`? Why or why not?
  - We can confidently conclude that POF !< 0.03 because the upper confidence interval is 0.046, which is above this limit.
